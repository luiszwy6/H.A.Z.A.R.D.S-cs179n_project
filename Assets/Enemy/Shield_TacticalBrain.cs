...
public class Shield_TacticalBrain : MonoBehaviour
{
    ...
    private class CombatStrafeRule
    {
        ...
        [Header("Distance")]
        [Min(0f)] public float minDistanceFromTarget = 1.2f;
        [Min(0f)] public float maxDistanceFromTarget = 5f;

        [Header("Movement")]
        [Min(0f)] public float minStrafeDistance = 1f;
        [Min(0f)] public float maxStrafeDistance = 2f;
        [Min(0f)] public float reachDistance = 0.75f;
    ...
    private class FollowShieldRule
    {
        ...
        [Min(0f)] public float sideOffset = 0.75f;
        ...
        [Header("Timing")]
        [Min(0.02f)] public float refreshInterval = 0.25f;
    ...
    private class ShieldWalkRule
    {
        ...
        [Header("Distance")]
        [Min(0f)] public float minDistanceFromTarget = 1f;
        [Min(0f)] public float maxDistanceFromTarget = 5f;
        [Min(0f)] public float activeMinDistanceBuffer = 0.5f;
        [Min(0f)] public float activeMaxDistanceBuffer = 1f;

        [Header("Movement")]
        [Min(0f)] public float minSideStepDistance = 1f;
        [Min(0f)] public float maxSideStepDistance = 3f;
        [Min(0f)] public float reachDistance = 0.75f;
        [Min(0.02f)] public float refreshInterval = 0.4f;

        [Header("Wait At Point")]
        [Min(0f)] public float minWaitAtPointTime = 5f;
        [Min(0f)] public float maxWaitAtPointTime = 8f;

        [Header("Facing")]
        public bool faceTargetWhileActive = true;
        [Min(0f)] public float faceRotationSpeed = 12f;
        ...
        [Header("Debug")]
        public bool debugLogShieldWalk = false;
    }

    [System.Serializable]
    private class ShieldAttackRule
    {
        [Header("Enable")]
        public bool enabled = true;

        [Header("Trigger")]
        public bool attackWhenARAndShotgunDead = true;
        public bool attackWhenPlayerReloading = true;
        public bool attackWhenPlayerInCover = true;
        public bool requireTargetVisible = true;

        [Header("Melee Approach")]
        [Min(0f)] public float meleeStopDistance = 1.6f;
        [Min(0f)] public float reachDistance = 0.75f;
        ...
        [Header("Debug")]
        public bool debugLogShieldAttack = false;
    }
    ...
    [Min(0f)] [SerializeField] private float attackRange = 3f;
    [Min(0f)] [SerializeField] private float tooCloseRange = 1.2f;
    [Min(0f)] [SerializeField] private float preferredRange = 2.2f;
    [Min(0f)] [SerializeField] private float chaseRange = 5f;
    [Min(0f)] [SerializeField] private float stopRange = 2.7f;
    [Min(0f)] [SerializeField] private float stopTolerance = 0.5f;
    [Min(0f)] [SerializeField] private float safeRange = 2f;
    ...
    [Header("Shield Walk Rule")]
    [SerializeField] private ShieldWalkRule shieldWalkRule = new ShieldWalkRule();

    [Header("Shield Attack Rule")]
    [SerializeField] private ShieldAttackRule shieldAttackRule = new ShieldAttackRule();
    ...
    private float nextShieldWalkUpdateTime;
    private float shieldWalkWaitEndTime = -1f;
    private float nextShieldAttackUpdateTime;
    ...
    private void Update()
    {
        ...
        if (TryUpdateShieldAttackTarget(PlayerStatus.Instance))
            return;

        if (TickActiveCombatStrafe())
            return;

        if (TickActiveShieldWalk())
            return;
    ...
    private bool TickTacticalTargetFailsafe()
    {
        ...
        if (currentReason == TacticalMoveReason.ShieldAttack &&
            ShouldShieldAttack(PlayerStatus.Instance))
        {
            lastTacticalTargetUpdateTime = Time.time;
            return false;
        }
    ...
    private bool TickActiveEscapeGrenade()
    {
        ...
        FaceAndAimAtPlayer(PlayerStatus.Instance, ResolveEscapeFacingRotationSpeed());
    ...
    private float ResolveEscapeFacingRotationSpeed()
    {
        if (shieldWalkRule != null)
            return shieldWalkRule.faceRotationSpeed;
    ...
    private void EvaluateTacticalDecision()
    {
        ...
        if (TryUpdateShieldAttackTarget(playerStatus))
            return;
        ...
        if (TryUpdateShieldWalkTarget(playerStatus))
            return;
    ...
    private bool TryStartLastKnownPositionSearch()
    {
        ...
        if (checkLastKnownPositionRule.debugLogCheckPosition)
            Debug.Log($"[Shield_TacticalBrain] {name} checking LKP move point={movePosition}", this);
    ...
    private bool TryFindLastKnownPatrolPosition(out Vector3 patrolPosition)
    {
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            if (patrolLastKnownPositionRule.debugLogPatrol)
                Debug.Log($"[Shield_TacticalBrain] {name} LKP patrol move point={patrolPosition}", this);
    ...
    private bool TryUpdateShieldAttackTarget(PlayerStatus playerStatus)
    {
        if (!ShouldShieldAttack(playerStatus))
        {
            if (currentReason == TacticalMoveReason.ShieldAttack)
                ClearTacticalTarget();
        ...
        if (currentReason == TacticalMoveReason.ShieldAttack &&
            ...
            Time.time < nextShieldAttackUpdateTime &&
            !HasReachedShieldAttackPoint())
        {
            return true;
        }

        if (!TryFindShieldAttackPosition(playerStatus, out Vector3 attackPosition))
            return false;

        SetTacticalMoveTarget(attackPosition, TacticalMoveReason.ShieldAttack);
        nextShieldAttackUpdateTime = Time.time + ResolveShieldAttackRefreshInterval();

        if (shieldAttackRule.debugLogShieldAttack)
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} shield attack move point={attackPosition}",
                this
            );
        }
    ...
    private bool ShouldShieldAttack(PlayerStatus playerStatus)
    {
        if (shieldAttackRule == null || !shieldAttackRule.enabled)
            return false;

        if (playerStatus == null)
            return false;

        if (shieldAttackRule.requireTargetVisible && !CanUseVisibleTarget())
            return false;

        bool shouldAttack = false;

        if (shieldAttackRule.attackWhenPlayerReloading && playerStatus.IsReloading)
            shouldAttack = true;

        if (shieldAttackRule.attackWhenPlayerInCover && playerStatus.IsInCover)
            shouldAttack = true;

        if (shieldAttackRule.attackWhenARAndShotgunDead && AreARAndShotgunDead())
            shouldAttack = true;

        return shouldAttack;
    }

    private bool AreARAndShotgunDead()
    {
        if (squadMember == null)
            squadMember = GetComponent<SquadMember>();
        ...
        return squadMember.SquadManager.CountAliveMembers(SquadEnemyType.AR) <= 0 &&
               squadMember.SquadManager.CountAliveMembers(SquadEnemyType.Shotgun) <= 0;
    }

    private bool HasReachedShieldAttackPoint()
    {
        if (!hasTacticalMoveTarget)
            return false;

        float reachDistance = shieldAttackRule != null
            ? Mathf.Max(0f, shieldAttackRule.reachDistance)
            : 0.75f;

        return Vector3.Distance(transform.position, tacticalMovePosition) <= reachDistance;
    }

    private bool TryFindShieldAttackPosition(PlayerStatus playerStatus, out Vector3 attackPosition)
    {
        ...
        float meleeStopDistance = shieldAttackRule != null
            ? Mathf.Max(0f, shieldAttackRule.meleeStopDistance)
            : 1.6f;

        Vector3 awayFromPlayer = transform.position - playerStatus.transform.position;
        awayFromPlayer.y = 0f;

        if (awayFromPlayer.sqrMagnitude <= 0.01f)
            awayFromPlayer = -playerStatus.transform.forward;
        ...
        if (currentDistance <= meleeStopDistance)
        {
            attackPosition = transform.position;
            return true;
        }
        ...
        Vector3 candidatePosition =
            playerStatus.transform.position + awayFromPlayer * meleeStopDistance;

        float sampleRadius = shieldAttackRule != null
            ? Mathf.Max(0f, shieldAttackRule.navMeshSampleRadius)
            : 1.5f;

        if (!TrySampleNavMeshPosition(candidatePosition, sampleRadius, out attackPosition))
            return false;

        if (shieldAttackRule != null &&
            shieldAttackRule.requireNavMeshReachable &&
            !IsNavMeshReachable(attackPosition))
        {
            return false;
        }
    ...
    private float ResolveShieldAttackRefreshInterval()
    {
        if (shieldAttackRule == null)
            return 0.2f;

        return Mathf.Max(0.02f, shieldAttackRule.refreshInterval);
    }
    ...
    private bool TryUpdateBackAwayTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} back away move point={backAwayPosition}",
                this
            );
        }
    ...
    private bool TryUpdateCombatStrafeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} combat strafe move point={strafePosition}",
                this
            );
        }
    ...
    private bool TickActiveCombatStrafe()
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} combat strafe next move point={strafePosition}",
                this
            );
        }
    ...
    {
        float min = combatStrafeRule != null ? Mathf.Max(0f, combatStrafeRule.minStrafeDistance) : 1f;
        float max = combatStrafeRule != null ? Mathf.Max(0f, combatStrafeRule.maxStrafeDistance) : 2f;
    ...
    {
        bool avoidTeammates = combatStrafeRule != null && combatStrafeRule.avoidTeammates;
        float avoidDistance = combatStrafeRule != null
            ? Mathf.Max(0f, combatStrafeRule.teammateAvoidDistance)
            : 0f;

        return IsTooCloseToTeammate(position, avoidTeammates, avoidDistance);
    }

    private bool TryUpdateShieldWalkTarget(PlayerStatus playerStatus)
    {
        if (!IsShieldWalkAllowed(playerStatus))
        {
            if (currentReason == TacticalMoveReason.ShieldWalk)
                ClearTacticalTarget();

            return false;
        }

        FaceShieldWalkTarget(playerStatus);

        if (currentReason == TacticalMoveReason.ShieldWalk &&
            hasTacticalMoveTarget &&
            Time.time < nextShieldWalkUpdateTime &&
            !HasReachedShieldWalkPoint())
        {
            return true;
        }

        if (!TryFindShieldWalkPosition(playerStatus, out Vector3 shieldWalkPosition))
            return false;

        SetTacticalMoveTarget(shieldWalkPosition, TacticalMoveReason.ShieldWalk);
        nextShieldWalkUpdateTime = Time.time + Mathf.Max(0.02f, shieldWalkRule.refreshInterval);

        if (shieldWalkRule.debugLogShieldWalk)
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} shield walk move point={shieldWalkPosition}",
                this
            );
        }
    ...
    private bool TickActiveShieldWalk()
    {
        if (currentReason != TacticalMoveReason.ShieldWalk)
            return false;
        ...
        if (!HasReachedShieldWalkPoint())
            return true;

        if (shieldWalkWaitEndTime < 0f)
        {
            shieldWalkWaitEndTime = Time.time + ResolveRandomShieldWalkWaitTime();
            return true;
        }

        if (Time.time < shieldWalkWaitEndTime)
            return true;

        shieldWalkWaitEndTime = -1f;
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} shield walk next move point={shieldWalkPosition}",
                this
            );
        }
    ...
    private bool IsShieldWalkAllowed(PlayerStatus playerStatus)
    {
        if (shieldWalkRule == null || !shieldWalkRule.enabled)
            return false;

        if (playerStatus == null)
            return false;

        if (shieldWalkRule.requireTargetVisible && !CanUseVisibleTarget())
            return false;

        float currentDistance = Vector3.Distance(transform.position, playerStatus.transform.position);
        float preferredReadyDistance = ResolvePreferredRange() + ResolveStopTolerance();
        bool isActiveShieldWalk = currentReason == TacticalMoveReason.ShieldWalk;
        float minDistance = Mathf.Max(0f, shieldWalkRule.minDistanceFromTarget);
        float maxDistance = Mathf.Max(0f, shieldWalkRule.maxDistanceFromTarget);

        if (isActiveShieldWalk)
        {
            minDistance = Mathf.Max(0f, minDistance - Mathf.Max(0f, shieldWalkRule.activeMinDistanceBuffer));
            maxDistance += Mathf.Max(0f, shieldWalkRule.activeMaxDistanceBuffer);
            preferredReadyDistance += Mathf.Max(0f, shieldWalkRule.activeMaxDistanceBuffer);
        }
    ...
    private bool HasReachedShieldWalkPoint()
    {
        if (!hasTacticalMoveTarget)
            return false;

        float reachDistance = shieldWalkRule != null
            ? Mathf.Max(0f, shieldWalkRule.reachDistance)
            : 0.75f;

        return Vector3.Distance(transform.position, tacticalMovePosition) <= reachDistance;
    }

    private float ResolveRandomShieldWalkWaitTime()
    {
        if (shieldWalkRule == null)
            return 0f;

        float min = Mathf.Max(0f, shieldWalkRule.minWaitAtPointTime);
        float max = Mathf.Max(0f, shieldWalkRule.maxWaitAtPointTime);
    ...
    private bool TryFindShieldWalkPosition(PlayerStatus playerStatus, out Vector3 shieldWalkPosition)
    {
        shieldWalkPosition = transform.position;

        if (playerStatus == null)
            return false;

        int attempts = shieldWalkRule != null
            ? Mathf.Max(1, shieldWalkRule.maxPickAttempts)
            : 8;

        for (int i = 0; i < attempts; i++)
        {
            Vector3 candidatePosition = GetShieldWalkCandidate(playerStatus.transform.position, i);
            float sampleRadius = shieldWalkRule != null
                ? Mathf.Max(0f, shieldWalkRule.navMeshSampleRadius)
                : 2f;
            ...
            if (shieldWalkRule != null &&
                shieldWalkRule.requireNavMeshReachable &&
                !IsNavMeshReachable(sampledPosition))
            {
                continue;
            }

            bool avoidTeammates = shieldWalkRule != null && shieldWalkRule.avoidTeammates;
            float avoidDistance = shieldWalkRule != null
                ? Mathf.Max(0f, shieldWalkRule.teammateAvoidDistance)
                : 0f;

            if (IsTooCloseToTeammate(sampledPosition, avoidTeammates, avoidDistance))
                continue;

            shieldWalkPosition = sampledPosition;
            return true;
        }
    ...
    private Vector3 GetShieldWalkCandidate(Vector3 targetPosition, int attempt)
    {
        float min = shieldWalkRule != null ? Mathf.Max(0f, shieldWalkRule.minSideStepDistance) : 1f;
        float max = shieldWalkRule != null ? Mathf.Max(0f, shieldWalkRule.maxSideStepDistance) : 3f;
        ...
        return transform.position + sideDirection.normalized * side * distance;
    }

    private void FaceShieldWalkTarget(PlayerStatus playerStatus)
    {
        if (shieldWalkRule == null || !shieldWalkRule.faceTargetWhileActive)
            return;

        FaceAndAimAtPlayer(playerStatus, shieldWalkRule.faceRotationSpeed);
    }

    private void FaceAndAimAtPlayer(PlayerStatus playerStatus, float rotationSpeed)
    {
        if (playerStatus == null)
            return;

        SetShieldAiming(true);

        if (navMeshAgent != null && navMeshAgent.enabled)
            navMeshAgent.updateRotation = false;

        Vector3 direction = playerStatus.transform.position - transform.position;
        direction.y = 0f;
    ...
    private void SetShieldAiming(bool value)
    {
        if (animatorDriver == null)
            animatorDriver = GetComponent<EnemyAnimatorParameterDriver>();
    ...
    private bool IsTooCloseToTeammate(Vector3 position, bool avoidTeammates, float avoidDistance)
    {
        if (!avoidTeammates)
            return false;
    ...
    private bool TryUpdatePreferredRangeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} preferred range move point={preferredPosition}",
                this
            );
        }
        ...
        out Vector3 flankPosition)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} selected flank stage={stage}, position={flankPosition}, side={currentFlankSide}",
                this
            );
        }
    ...
    private void SetEnemyMeleeCombatingStatus(bool value)
    {
        EnemyStatus status = ResolveEnemyStatus();

        if (status != null)
            status.SetMeleeCombating(value);
    }
    ...
    private bool TryFindGrenadeEscapePosition(out Vector3 escapePosition)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} escaping grenade: {grenade.name}, type={grenade.GrenadeType}, position={escapePosition}",
                this
            );
        }
        ...
        float searchRadiusOverride)
    {
        ...
        {
            Debug.Log(
                $"[Shield_TacticalBrain] {name} selected cover: {bestCover.name}, position={coverPosition}",
                this
            );
        }
    ...
    public void SetTacticalMoveTarget(Vector3 worldPosition, TacticalMoveReason reason)
    {
        ...
        SetEnemyMeleeCombatingStatus(reason == TacticalMoveReason.ShieldAttack);
        lastKnownPatrolWaitEndTime = -1f;
    ...
    private bool ShouldApplySquadTacticalMoveOffset(TacticalMoveReason reason)
    {
        ...
        if (reason == TacticalMoveReason.ShieldWalk)
            return false;

        if (reason == TacticalMoveReason.ShieldAttack)
            return false;
    ...
    {
        TacticalMoveReason previousReason = currentReason;
        ...
        combatStrafeWaitEndTime = -1f;
        shieldWalkWaitEndTime = -1f;
        nextShieldAttackUpdateTime = 0f;
        lastKnownPatrolWaitEndTime = -1f;


        if (previousReason == TacticalMoveReason.EscapeGrenade ||
            previousReason == TacticalMoveReason.ShieldWalk)
        {
            SetShieldAiming(false);
        }
        ...
        SetEnemyMeleeCombatingStatus(false);
