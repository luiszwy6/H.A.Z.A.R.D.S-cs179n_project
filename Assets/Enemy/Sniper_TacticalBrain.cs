...
public class Sniper_TacticalBrain : MonoBehaviour
{
    ...
    private class CombatStrafeRule
    {
        ...
        [Header("Movement")]
        [Min(0f)] public float minStrafeDistance = 0.75f;
        [Min(0f)] public float maxStrafeDistance = 1.5f;
        [Min(0f)] public float reachDistance = 0.75f;
        [Min(0.02f)] public float refreshInterval = 0.4f;

        [Header("Wait At Point")]
        [Min(0f)] public float minWaitAtPointTime = 2.5f;
        [Min(0f)] public float maxWaitAtPointTime = 4.5f;
    ...
    private bool TryStartLastKnownPositionSearch()
    {
        ...
        if (checkLastKnownPositionRule.debugLogCheckPosition)
            Debug.Log($"[Sniper_TacticalBrain] {name} checking LKP move point={movePosition}", this);
    ...
    private bool TryFindLastKnownPatrolPosition(out Vector3 patrolPosition)
    {
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            if (patrolLastKnownPositionRule.debugLogPatrol)
                Debug.Log($"[Sniper_TacticalBrain] {name} LKP patrol move point={patrolPosition}", this);
    ...
    private bool TryUpdateBackAwayTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} back away move point={backAwayPosition}",
                this
            );
        }
    ...
    private bool TryUpdateCombatStrafeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} combat strafe move point={strafePosition}",
                this
            );
        }
    ...
    private bool TickActiveCombatStrafe()
    {
        ...
        if (combatStrafeWaitEndTime < 0f)
        {
            ...
            nextCombatStrafeUpdateTime = combatStrafeWaitEndTime;
            return true;
        }
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} combat strafe next move point={strafePosition}",
                this
            );
        }
    ...
    private bool TryUpdatePreferredRangeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} preferred range move point={preferredPosition}",
                this
            );
        }
    ...
    private bool TryFindFollowShieldPosition(out Vector3 followPosition)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} following shield: {shieldMember.name}, position={followPosition}",
                this
            );
        }
        ...
        out Vector3 flankPosition)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} selected flank stage={stage}, position={flankPosition}, side={currentFlankSide}",
                this
            );
        }
    ...
    private bool TryFindGrenadeEscapePosition(out Vector3 escapePosition)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} escaping grenade: {grenade.name}, type={grenade.GrenadeType}, position={escapePosition}",
                this
            );
        }
        ...
        float searchRadiusOverride)
    {
        ...
        {
            Debug.Log(
                $"[Sniper_TacticalBrain] {name} selected cover: {bestCover.name}, position={coverPosition}",
                this
            );
        }
