...
public class SG_TacticalBrain : MonoBehaviour
{
    ...
    [System.Serializable]
    private class ShieldCoverRule
    {
        [Header("Enable")]
        public bool enabled = true;

        [Header("Position")]
        [Min(0f)] public float behindShieldDistance = 2f;
        ...
        [Header("Debug")]
        public bool debugLogSelectedShield = false;
    }
    ...
    private class CombatStrafeRule
    {
        ...
        [Header("Distance")]
        [Min(0f)] public float minDistanceFromTarget = 2.5f;
        [Min(0f)] public float maxDistanceFromTarget = 12f;

        [Header("Movement")]
        [Min(0f)] public float minStrafeDistance = 1.2f;
        [Min(0f)] public float maxStrafeDistance = 3f;
        [Min(0f)] public float reachDistance = 0.75f;
    ...
    [Min(0f)] [SerializeField] private float attackRange = 12f;
    [Min(0f)] [SerializeField] private float tooCloseRange = 1.5f;
    [Min(0f)] [SerializeField] private float preferredRange = 3f;
    [Min(0f)] [SerializeField] private float chaseRange = 4.5f;
    [Min(0f)] [SerializeField] private float stopRange = 3.75f;
    [Min(0f)] [SerializeField] private float stopTolerance = 0.75f;
    [Min(0f)] [SerializeField] private float safeRange = 1.5f;
    ...
    [Header("Shotgun Aim Control")]
    [SerializeField] private bool facePlayerWhileAiming = true;
    [Min(0f)] [SerializeField] private float facePlayerRotationSpeed = 12f;
    [SerializeField] private bool disableAgentRotationWhileAiming = true;

    [Header("Take Cover Rule")]
    [SerializeField] private TakeCoverRule takeCoverRule = new TakeCoverRule();

    [Header("Shield Cover Rule")]
    [SerializeField] private ShieldCoverRule shieldCoverRule = new ShieldCoverRule();
    ...
    private void MaintainShotgunAimingState()
    {
        bool shouldAim = !ShouldSuppressShotgunAiming();

        SetShotgunAiming(shouldAim);

        if (shouldAim)
            FacePlayerWhileAiming();
    }

    private bool ShouldSuppressShotgunAiming()
    {
        ...
        return weaponSettings != null && weaponSettings.IsReloading;
    }

    private void SetShotgunAiming(bool value)
    {
        if (animatorDriver == null)
            animatorDriver = GetComponent<EnemyAnimatorParameterDriver>();
    ...
    private void FacePlayerWhileAiming()
    {
        if (!facePlayerWhileAiming)
            return;
        ...
        if (navMeshAgent != null &&
            ...
            disableAgentRotationWhileAiming)
        {
        ...
        transform.rotation = Quaternion.Slerp(
            transform.rotation,
            targetRotation,
            Mathf.Max(0f, facePlayerRotationSpeed) * Time.deltaTime
        );
    }
    ...
    private bool TickFollowShield()
    {
        ...
        if (Time.time < nextFollowShieldUpdateTime &&
            currentReason == TacticalMoveReason.FollowShield &&
    ...
    private void EvaluateTacticalDecision()
    {
        ...
        if (ShouldTakeCoverForReload())
        {
            ...
            if (TryFindTakeCoverPosition(playerStatus, out coverPosition))
            {
    ...
    private bool TryStartLastKnownPositionSearch()
    {
        ...
        if (checkLastKnownPositionRule.debugLogCheckPosition)
            Debug.Log($"[SG_TacticalBrain] {name} checking LKP move point={movePosition}", this);
    ...
    private bool TryFindLastKnownPatrolPosition(out Vector3 patrolPosition)
    {
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            if (patrolLastKnownPositionRule.debugLogPatrol)
                Debug.Log($"[SG_TacticalBrain] {name} LKP patrol move point={patrolPosition}", this);
    ...
    private bool TryUpdateBackAwayTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} back away move point={backAwayPosition}",
                this
            );
        }
    ...
    private bool TryUpdateCombatStrafeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} combat strafe move point={strafePosition}",
                this
            );
        }
    ...
    private bool TickActiveCombatStrafe()
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} combat strafe next move point={strafePosition}",
                this
            );
        }
    ...
    {
        float min = combatStrafeRule != null ? Mathf.Max(0f, combatStrafeRule.minStrafeDistance) : 1.2f;
        float max = combatStrafeRule != null ? Mathf.Max(0f, combatStrafeRule.maxStrafeDistance) : 3f;
    ...
    private bool TryUpdatePreferredRangeTarget(PlayerStatus playerStatus)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} preferred range move point={preferredPosition}",
                this
            );
        }
        ...
        out Vector3 flankPosition)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} selected flank stage={stage}, position={flankPosition}, side={currentFlankSide}",
                this
            );
        }
    ...
    private bool TryFindGrenadeEscapePosition(out Vector3 escapePosition)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} escaping grenade: {grenade.name}, type={grenade.GrenadeType}, position={escapePosition}",
                this
            );
        }
    ...
    private bool TryFindTakeCoverPosition(PlayerStatus playerStatus, out Vector3 coverPosition)
    {
        if (TryUseCurrentCoverPositionIfAlreadyInCover(out coverPosition))
            return true;

        if (TryFindShieldCoverPosition(playerStatus, out coverPosition))
        {
            suppressNextSquadTacticalMoveOffset = true;
            return true;
        }
        ...
        return TryFindCoverPosition(playerStatus, out coverPosition);
    }
    ...
    private bool TryFindFollowShieldPosition(out Vector3 followPosition)
    {
        ...
        if (!TryGetBestAliveShield(out SquadMember shield))
            return false;

        Vector3 referencePosition = ResolveShieldReferencePosition(shield.transform);

        if (!TryGetShieldBehindDirection(shield.transform, referencePosition, out Vector3 behindDirection))
            return false;

        Vector3 sideDirection = Vector3.Cross(Vector3.up, behindDirection);

        if (sideDirection.sqrMagnitude <= 0.01f)
            sideDirection = shield.transform.right;
        ...
        if (followShieldTarget != shield || followShieldSide == 0)
        {
            followShieldTarget = shield;
            followShieldSide = ResolveInitialFollowShieldSide(shield);
        }

        float[] sideOrder = new float[] { followShieldSide, -followShieldSide };

        for (int sideIndex = 0; sideIndex < sideOrder.Length; sideIndex++)
        {
            Vector3 candidatePosition =
                shield.transform.position +
                behindDirection * Mathf.Max(0f, followShieldRule.behindShieldDistance) +
                sideDirection * sideOrder[sideIndex] * Mathf.Max(0f, followShieldRule.sideOffset);

            if (!TryValidateFollowShieldPosition(candidatePosition, out candidatePosition))
                continue;

            if (sideIndex > 0)
                followShieldSide = -followShieldSide;

            followPosition = candidatePosition;

            if (followShieldRule.debugLogFollowShield)
            {
                Debug.Log(
                    $"[SG_TacticalBrain] {name} following shield {shield.name}, position={followPosition}",
                    this
                );
            }
    ...
    private bool TryGetBestAliveShield(out SquadMember shield)
    {
        shield = null;
        ...
        IReadOnlyList<SquadMember> shieldMembers = squadMember.SquadManager.ShieldMembers;

        if (shieldMembers == null || shieldMembers.Count == 0)
            return false;

        float bestDistance = float.MaxValue;

        for (int i = 0; i < shieldMembers.Count; i++)
        {
            SquadMember candidate = shieldMembers[i];

            if (candidate == null || candidate == squadMember || !candidate.IsAlive)
                continue;
    ...
    private bool HasAliveShieldToFollow()
    {
        return followShieldRule != null &&
               followShieldRule.enabled &&
               TryGetBestAliveShield(out SquadMember ignoredShield);
    }
    ...
    private int ResolveInitialFollowShieldSide(SquadMember shield)
    {
        if (followShieldRule != null && followShieldRule.randomizeInitialSide)
            return Random.value < 0.5f ? -1 : 1;

        int seed = GetInstanceID();

        if (shield != null)
            seed += shield.GetInstanceID();

        return Mathf.Abs(seed) % 2 == 0 ? -1 : 1;
    }
    ...
    {
        followPosition = inputPosition;

        float sampleRadius = followShieldRule != null
            ? followShieldRule.navMeshSampleRadius
            : 1.5f;

        if (!TrySampleNavMeshPosition(inputPosition, sampleRadius, out followPosition))
            return false;

        if (followShieldRule != null && followShieldRule.requireNavMeshReachable)
        {
            if (!IsNavMeshReachable(followPosition))
                return false;
        }
    ...
    private bool TryFindShieldCoverPosition(PlayerStatus playerStatus, out Vector3 coverPosition)
    {
        coverPosition = transform.position;

        if (shieldCoverRule == null || !shieldCoverRule.enabled)
            return false;
        ...
        SquadMember bestShield = null;
        Vector3 bestPosition = transform.position;
        float bestScore = float.MaxValue;

        for (int i = 0; i < shieldMembers.Count; i++)
        {
            SquadMember shield = shieldMembers[i];

            if (shield == null || shield == squadMember || !shield.IsAlive)
                continue;

            if (!TryGetShieldBehindDirection(shield.transform, playerPosition, out Vector3 behindDirection))
                continue;
            ...
            float firstSide = ResolveShieldCoverSide(shield, i);
            float[] sideOrder = new float[] { firstSide, -firstSide };

            for (int sideIndex = 0; sideIndex < sideOrder.Length; sideIndex++)
            {
                Vector3 candidatePosition =
                    shield.transform.position +
                    behindDirection * Mathf.Max(0f, shieldCoverRule.behindShieldDistance) +
                    sideDirection * sideOrder[sideIndex] * Mathf.Max(0f, shieldCoverRule.sideOffset);

                if (!TryValidateShieldCoverPosition(candidatePosition, playerPosition, out candidatePosition))
                    continue;

                float score = Vector3.Distance(transform.position, candidatePosition);
        ...
        if (shieldCoverRule.debugLogSelectedShield)
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} selected shield cover behind {bestShield.name}, position={coverPosition}",
                this
            );
        }
        ...
        out Vector3 behindDirection)
    {
        ...
        behindDirection = shieldTransform.position - playerPosition;
    ...
    private float ResolveShieldCoverSide(SquadMember shield, int shieldIndex)
    {
        if (shieldCoverRule != null && shieldCoverRule.randomizeSide)
        ...
        return Mathf.Abs(seed) % 2 == 0 ? -1f : 1f;
    }

    private bool TryValidateShieldCoverPosition(
        Vector3 inputPosition,
        Vector3 playerPosition,
        out Vector3 coverPosition)
    {
        coverPosition = inputPosition;

        if (takeCoverRule != null)
        {
            float distanceFromPlayer = Vector3.Distance(playerPosition, inputPosition);

            if (distanceFromPlayer < takeCoverRule.minDistanceFromPlayer)
                return false;
        }

        float sampleRadius = shieldCoverRule != null
            ? shieldCoverRule.navMeshSampleRadius
            : 1.5f;

        if (!TrySampleNavMeshPosition(inputPosition, sampleRadius, out coverPosition))
            return false;

        if (shieldCoverRule != null && shieldCoverRule.requireNavMeshReachable)
        {
            if (!IsNavMeshReachable(coverPosition))
                return false;
        }
        ...
        float searchRadiusOverride)
    {
        ...
        {
            Debug.Log(
                $"[SG_TacticalBrain] {name} selected cover: {bestCover.name}, position={coverPosition}",
                this
            );
        }
