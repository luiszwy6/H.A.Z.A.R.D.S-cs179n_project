...
public class HitPartShakeReaction : MonoBehaviour
{
    ...
    [Header("Stun Rules")]
    [SerializeField] private bool blockShakeWhileEnemyStunned = true;
    [SerializeField] private EnemyStunReceiver stunReceiver;
    [SerializeField] private bool autoFindStunReceiver = true;
    ...
    private bool ShouldBlockShakeNow()
    {
        if (!blockShakeWhileEnemyStunned)
            return false;

        ResolveReferences();

        return stunReceiver != null && stunReceiver.IsStunned;
    }

    private void ResolveReferences()
    {
        if (!autoFindStunReceiver)
            return;
