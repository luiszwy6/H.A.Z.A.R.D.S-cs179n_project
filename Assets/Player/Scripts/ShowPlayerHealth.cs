using UnityEngine;

public class ShowPlayerHealth : MonoBehaviour
{
    [Header("References")]
    [SerializeField] private PlayerHealth playerHealth;

    [Header("Display")]
    [SerializeField] private bool show = true;
    [SerializeField] private Vector2 offsetFromBottomLeft = new Vector2(18f, 18f);
    [SerializeField] private float boxWidth = 220f;
    [SerializeField] private float boxHeight = 64f;
    [SerializeField] private int fontSize = 22;
    [SerializeField] private string title = "Health";

    private GUIStyle boxStyle;
    private GUIStyle titleStyle;
    private GUIStyle textStyle;
    private GUIStyle lowHealthStyle;
    private bool stylesReady;

    private void Awake()
    {
        if (playerHealth == null)
            playerHealth = GetComponent<PlayerHealth>();
    }

    private void EnsureStyles()
    {
        if (stylesReady)
            return;

        boxStyle = new GUIStyle(GUI.skin.box);
        boxStyle.alignment = TextAnchor.UpperLeft;
        boxStyle.padding = new RectOffset(10, 10, 8, 8);

        titleStyle = new GUIStyle(GUI.skin.label);
        titleStyle.fontSize = fontSize;
        titleStyle.fontStyle = FontStyle.Bold;

        textStyle = new GUIStyle(GUI.skin.label);
        textStyle.fontSize = fontSize;

        lowHealthStyle = new GUIStyle(textStyle);
        lowHealthStyle.normal.textColor = Color.red;

        stylesReady = true;
    }

    private void OnGUI()
    {
        if (!show || playerHealth == null)
            return;

        EnsureStyles();

        float current = playerHealth.CurrentHealth;
        float max     = playerHealth.BaseHealth;
        float ratio   = max > 0f ? current / max : 0f;
        bool lowHealth = ratio <= 0.3f;

        Rect boxRect = new Rect(
            offsetFromBottomLeft.x,
            Screen.height - offsetFromBottomLeft.y - boxHeight,
            boxWidth,
            boxHeight
        );

        GUI.Box(boxRect, GUIContent.none, boxStyle);

        Rect titleRect = new Rect(boxRect.x + 8f, boxRect.y + 4f,  boxRect.width - 16f, 24f);
        Rect valueRect = new Rect(boxRect.x + 8f, boxRect.y + 30f, boxRect.width - 16f, 28f);

        GUI.Label(titleRect, title, titleStyle);
        GUI.Label(valueRect, $"{current:F0} / {max:F0}", lowHealth ? lowHealthStyle : textStyle);
    }
}
