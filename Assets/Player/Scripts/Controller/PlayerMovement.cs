...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("View Driver")]
    [SerializeField] private PlayerMovementViewBase activeView;
    [SerializeField] private PlayerTopDownView defaultTopDownView;
    ...
    [Header("Slide Aim")]
    [SerializeField] private bool allowAimDuringSlide = true;
    [SerializeField] private bool updateAimWhileSliding = true;
    [SerializeField] private bool rotateToAimWhileSliding = true;
    [SerializeField] private bool allowAimCancelSlideOnlyWhenSlideAimDisabled = true;
    ...
    private bool slideExitRequestedRun;
    ...
    public PlayerMovementViewBase ActiveView => activeView;
    ...
    public bool CanShootNow =>
        !isDiving &&
        !isRunningState &&
        (!isSliding || allowQuickShotDuringSlide || allowAimShootDuringSlide);

    public bool CanAimShootNow =>
        !isDiving &&
        !isRunningState &&
        (!isSliding || allowAimShootDuringSlide);
    ...
    void Awake()
    {
        ...
        if (defaultTopDownView == null)
            defaultTopDownView = GetComponent<PlayerTopDownView>();
    ...
    public void CancelAimAndRequireRepress()
    {
        ...
        if (activeView != null)
            activeView.CancelAimAndRequireRepress();
    ...
    void Update()
    {
        ...
        PlayerMovementViewFrame viewFrame = BuildViewFrame(
            moveInput,
            isCrouching || isProne
        );

        float inputMagnitude = viewFrame.InputMagnitude;
        bool wantsToMove = viewFrame.WantsToMove;
        ...
        if (!externalMovementLock && !useRootMotionRotation && desiredFacingDir.sqrMagnitude > 0.0001f)
        {
            ...
            if (activeView != null &&
                activeView.TryGetExternalAimRotationSpeedOverride(out float viewOverrideRotationSpeed))
            {
                activeRotationSpeed = viewOverrideRotationSpeed;
            }
            else if (aimSettings != null &&
    ...
    PlayerMovementViewFrame BuildViewFrame(Vector2 moveInput, bool isCrouchingOrProne)
    {
        float inputMagnitude = Mathf.Clamp01(moveInput.magnitude);
        bool wantsToMove = inputMagnitude > 0.1f;

        Vector3 fallbackMoveDirWorld = cameraTransform != null
            ? PlayerMovementViewUtility.GetCameraRelativeMoveDirection(moveInput, cameraTransform)
            : PlayerMovementViewUtility.GetWorldInputDirection(moveInput);

        if (!wantsToMove)
            fallbackMoveDirWorld = Vector3.zero;

        if (activeView != null)
        {
            return activeView.BuildViewFrame(
                transform,
                moveInput,
    ...
    {
        UpdateSlideExitRunRequest();

        bool slideAimActive = false;
        Vector3 slideAimFacingDir = Vector3.zero;

        if (updateAimWhileSliding)
        {
            Vector2 moveInput = moveAction != null ? moveAction.ReadValue<Vector2>() : Vector2.zero;
            ...
            {
                slideAimFacingDir = viewFrame.FacingDir;
                slideAimFacingDir.y = 0f;

                if (slideAimFacingDir.sqrMagnitude > 0.0001f)
                {
                    slideAimFacingDir.Normalize();
                    slideAimActive = true;
                    desiredFacingDir = slideAimFacingDir;
                }
            }
            ...
            if (allowAimCancelSlide &&
                (!allowAimCancelSlideOnlyWhenSlideAimDisabled || !allowAimDuringSlide) &&
                aimPressedThisFrame)
            {
                CancelSlideIntoAim();
                return;
        ...
        else if (allowAimCancelSlide && aimSettings != null && aimSettings.AimInputPressedThisFrame)
        {
            CancelSlideIntoAim();
            return;
        }
        ...
        Vector3 rotationDir = slideDirection;

        if (rotateToAimWhileSliding && slideAimActive && slideAimFacingDir.sqrMagnitude > 0.0001f)
            rotationDir = slideAimFacingDir;

        if (rotationDir.sqrMagnitude > 0.0001f)
        {
            float activeRotationSpeed = slideRotationSpeed;
            ...
            Quaternion targetRot = Quaternion.LookRotation(rotationDir, Vector3.up);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRot, activeRotationSpeed * dt);
        }
    ...
    void UpdateSlideExitRunRequest()
    {
        slideExitRequestedRun = false;

        if (!allowSlideExitToRun)
            return;
    ...
    {
        bool exitToRun = allowSlideExitToRun && slideExitRequestedRun;

        isSliding = false;
        isProne = false;
        isCrouching = exitToRun ? false : slideExitToCrouchWhenNotRunning;
        isAiming = false;
        isRunningState = exitToRun;
        isWalkingState = false;
        externalMovementLock = false;
        slideExitRequestedRun = false;
        ...
        if (animator != null)
        {
            ...
            animator.SetBool("IsProne", false);
            animator.SetBool("IsCrouching", isCrouching);
            animator.SetBool("IsRunning", exitToRun);
            animator.SetBool("IsWalking", false);
            animator.SetBool("IsMoving", exitToRun);
            animator.SetBool("IsAiming", false);
            animator.SetBool("IsIdle", !exitToRun && !isCrouching);
        }

        if (applyRecoveryLock && !exitToRun)
